...
- **Specs engine (Chunk 6)** — Channel adapter layer. Extracted shared message handling (blank check, orchestrator call, error rescue) into `Specs::MessageHandling` concern, replacing duplicated code across admin, client, and guest controllers. `Specs::Adapters` registry with `Base` and `Web` adapters wrapping orchestrator calls. `channel_type` (web/slack/teams) and `channel_metadata` (JSONB) columns on sessions for future multi-channel support. Guest controller now validates blank messages (bug fix). 389 total specs, all passing.
